...
- Since Slice 5a (issue #12) the response is ported, and the `record` policy is gone. The response runs behind the same trace line, so this slice's fixtures are unchanged; its own boundary is `create_supply_task`, which always fails loudly outside tests (`docs/slices/force-low-on-supplies.md`).
