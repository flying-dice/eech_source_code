...
{
	snprintf (labels[(int) get_local_entity_index (en)], sizeof (labels[0]), "%s", label);
}
...
static entity *find_created (const char *label)
{
	...
	{
		if (strcmp (labels[(int) get_local_entity_index (keysites[i])], label) == 0)
		{
			return keysites[i];
		}
	}
	...
	{
		if (strcmp (labels[(int) get_local_entity_index (en)], label) == 0)
		{
			return en;
		}
	}
...
static void label_new_tasks (void)
{
	...
	for (en = first_used_entity; en; en = en->succ)
	{
		if ((get_local_entity_type (en) == ENTITY_TYPE_TASK) && (labels[(int) get_local_entity_index (en)][0] == '\0'))
		{
			snprintf (labels[(int) get_local_entity_index (en)], sizeof (labels[0]), "task%d", (int) get_local_entity_index (en));
		}
	}
}
...
static void print_lifecycle_state (void)
{
	...
	{
		eech_out (" %d", (int) get_local_entity_index (en));
	}
...
static void label_sectors (void)
{
	...
	for (z = MIN_MAP_Z_SECTOR; z <= MAX_MAP_Z_SECTOR; z++)
	{
		for (x = MIN_MAP_X_SECTOR; x <= MAX_MAP_X_SECTOR; x++)
		{
			...
			snprintf (labels[(int) get_local_entity_index (en)], sizeof (labels[0]), "sector%d_%d", x, z);
		}
	}
}
 ...
 * for each shape of attribute list (up to five attributes of the four argument
 * patterns; the arguments of one attribute as the ENTITY_ATTR_* macros pass them) is generated by eech-sys/build into
...
static void run_line (char *line)
{
	...
	else if (strcmp (word, "update-cargo") == 0)
	{
		...
		for (en = first_used_entity; en; en = en->succ)
		{
			if ((get_local_entity_type (en) == ENTITY_TYPE_CARGO) && (labels[(int) get_local_entity_index (en)][0] == '\0'))
			{
				snprintf (labels[(int) get_local_entity_index (en)], sizeof (labels[0]), "crate%d", (int) get_local_entity_index (en));
			}
		}
	...
	else if ((strcmp (word, "map") == 0) || (strcmp (word, "create") == 0) || (strcmp (word, "destroy") == 0) || (strcmp (word, "allocate") == 0))
	{
		...
		else if (strcmp (op, "create") == 0)
		{
			...
			if (en)
			{
				...
				eech_out ("created %s %d\n", label, (int) get_local_entity_index (en));
			}
			else
			{
...
static void write_all (const char *text, size_t length)
{
	while (length > 0)
	{
		...
		if (!WriteFile (GetStdHandle (STD_OUTPUT_HANDLE), text, (DWORD) length, &written, NULL) || (written == 0))
		{
			ExitProcess (3);
		}
#endif
...
/* the harness's outcome for a fault inside the NULL page */
static void report_null_dereference (void)
{
	int
		i;
...
static void segmentation_fault (int signal_number, siginfo_t *info, void *context)
{
	...
	{
		report_null_dereference ();

		_exit (0);
	}

	/* SA_RESETHAND restored the default action: any other fault kills the process */
}
...
/* Windows: an access violation reading or writing inside the NULL page */
static LONG WINAPI access_violation (EXCEPTION_POINTERS *info)
{
	if ((info->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) && (info->ExceptionRecord->NumberParameters >= 2) && (info->ExceptionRecord->ExceptionInformation[1] < 4096))
	{
		report_null_dereference ();
...
int eech_k_legacy_install_fault_handler (void)
{
	return AddVectoredExceptionHandler (1, access_violation) ? EECH_STATUS_OK : EECH_STATUS_FATAL;
}
